...
>
> - Software Process Model ( 소프트웨어 프로세스 모형 )
> - Software Engineering Paradigm ( 소프트웨어 공학 패러다임 )
...
| 기본 원리 | 설명 |
|-----------|------|
| 👥 Pair Programming ( 짝 프로그래밍 ) | 두 명의 개발자가 하나의 작업을 함께 수행 |
| 🤝 Collective Ownership ( 공동 코드 소유 ) | 모든 개발자가 모든 코드를 수정 가능 |
| 🔄 Continuous Integration ( 지속적인 통합 ) | 변경된 코드를 지속적으로 통합 |
| 📅 Planning Process ( 계획 세우기 ) | 고객과 함께 개발 계획 수립 |
| 🚀 Small Releases ( 작은 릴리즈 ) | 작은 단위로 자주 릴리즈 |
| 💭 Metaphor ( 메타포어 ) | 공통된 비유를 사용하여 시스템 이해 |
| ✨ Simple Design ( 간단한 디자인 ) | 현재 필요한 기능만 단순하게 설계 |
| 🧪 Test-Driven Development ( TDD, 테스트 기반 개발  ) | 테스트를 먼저 작성한 후 개발 |
| ♻️ Refactoring ( 리책토링 ) | 기능 변화 없이 내부 구조 개선 |
| ⏰ 40-Hour Week ( 40시간 작업 ) | 무리한 야근을 지양하는 개발 문화 |
| 🙋 On-Site Customer ( 고객 상주 ) | 고객이 개발팀과 함께 참여 |
| 📖 Code Standard ( 코드 표준 ) | 코딩 규칙을 통일하여 유지보수성 향상 |
